...
demands_df = pd.pivot_table(demands_df, index=["Products", "Markets", "Period"], values=["value"])
...
marginalProfit = mdl.sum(ExecutedSales[pro, mark, mo] * val
                         for (pro, mark, mo, val) in marginalProfits_df[marginalProfits_df['Period'].isin(horizon)].
...
for pmc in plantMonthCapacities_df[plantMonthCapacities_df["Period"].isin(horizon)].itertuples(index=False):
    mdl.add_constraint(mdl.sum(Production[pmc.Plants, pr, pmc.Period] for pr in products) <= pmc.value)
...
mdl.add_constraints(
    ExecutedSales[s.Products, s.Markets, s.Period] >= s.value
    for s in sales_df[(sales_df['value'] > 0) & (sales_df['Period'].isin(horizon))].itertuples(index=False))
...
TProduction = namedtuple("TProduction", ["Plants", "Period", "Products", "value"])
production = []
for pl in plants:
    for pr in products:
        for mo in horizon:
...
TExecutedSales = namedtuple("TExecutedSales", ["Markets", "Period", "Products", "value"])
executedsales = []
for ma in markets:
    for pr in products:
        for mo in horizon:
...
TInventory = namedtuple("TInventory", ["Markets", "Period", "Products", "value"])
inventory = []
for ma in markets:
    for pr in products:
        for mo in horizon:
